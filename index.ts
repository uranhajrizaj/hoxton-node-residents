...
app.patch("/residents/:id", (req, res) => {
  ...
  if (match) {
    ...
    if (req.body.age) match.age = Number(req.body.age);
    if (req.body.gender) match.gender = req.body.gender;
    if (req.body.houseID) {
      const findHouse = houses.find(
        (house) => house.id === Number(req.body.houseID)
      );
      console.log(findHouse);
      const houseResidents = residents.filter(
        (resident) => resident.houseID === Number(req.body.houseID)
      ).length;
...
app.patch("/houses/:id", (req, res) => {
  ...
  if (match) {
    ...
    if (req.body.capacity) match.capacity = req.body.capacity;
    res.send(match);
  } else res.status(404).send({ error: " House not found" });
});
...
app.post("/houses", (req, res) => {
  ...
  if (typeof req.body.adress !== "string")
    errors.push("Adress not provided or not a string");
